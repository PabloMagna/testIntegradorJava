...
public class ServletCuenta extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("idCliente") != null) {
			...
			ArrayList<Cuenta> cuentas = cuentaNegocio
					.ListarPorIdCliente(Integer.parseInt(request.getParameter("idCliente")));
			request.setAttribute("listaCuentas", cuentas);
		...
		if (request.getParameter("listaPorId") != null) {
			...
			if (cliente != null) {
				...
				// Guardar la lista de cuentas en el atributo "listaCuentas" para enviarla a la
				// vista
				request.setAttribute("listaCuentas", cuentas);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("btnAgregar") != null) {
	        ...
	        if (numeroCuenta == -1) {
	            // Establece la advertencia y permanece en la misma página
	            request.setAttribute("advertencia", "error");

	            // Obtener la lista de tipos de cuenta y otros datos necesarios
	            ArrayList<TipoCuenta> tiposCuenta = cuentaNegocio.ListarTipoCuenta(); // Asegúrate de implementar este método.
	            request.setAttribute("tiposCuenta", tiposCuenta);

	            // Puedes establecer otros atributos según sea necesario

	            // Despacha de nuevo a la misma página de alta
	            RequestDispatcher dispatcher = request.getRequestDispatcher("AltaCuenta.jsp");
	            dispatcher.forward(request, response);
